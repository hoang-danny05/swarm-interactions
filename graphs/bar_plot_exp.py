...
class freqencyDict():
    ...
    def generate_config_plot(self, config):
        
        # get sorted configuration dictionary 
        sorted_words = sorted(self.config_dict.items(), key=lambda item: item[1], reverse=True)
        sorted_config_dict = dict(sorted_words)

        self.display_dict(sorted_config_dict)

        keys, values = zip(*sorted_words[:50])
        ...
        plt.title(f"Word frequencies for {config}", fontweight="bold")
        fig.autofmt_xdate(rotation=45)
        # plt.xticks(rotation=90)


        path = f"./graphs/frequency_plots_exp/{config}.png"
        plt.savefig(path)
        plt.show()
    ...
    def generate_full_plot(self):
        ...
        plt.xlabel("Word", fontweight="bold")
        plt.ylabel("Frequency", fontweight="bold")
        plt.title("Word Frequencies Across Everything ", fontweight="bold")
        fig.autofmt_xdate(rotation=45)
        # plt.xticks(rotation=90)


        path = f"./graphs/frequency_plots_exp/FULL.png"
        plt.savefig(path)
        plt.show()
...
def main():
    ...
    for conf in itertools.product(configs, configs):
        ...
        frequency_manager.generate_config_plot(config)
    frequency_manager.generate_full_plot()
